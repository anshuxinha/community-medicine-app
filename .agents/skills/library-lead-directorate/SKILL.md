...
description: >
  ...
  to overhaul, clean, elevate, and verify any Library content for MD Community Medicine.
  Spawns as many dedicated worker sub-agents as required for parallel chapter/section
  cleaning, subjects all drafts to an automated preflight audit and three-way qualitative
  directorate sign-off, and strictly prohibits shipping until all 3 lead agents approve.
  Enforces the full ship protocol: Firebase libraryContentOverrides update, Git commit/push,
  and production EAS OTA update with manual test checklist. Use when the user invokes
  /library-directorate, /lead-directorate, /directorate-audit, /directorate-overhaul,
  "run lead directorate", "run the 3 lead agents", "overhaul library content with lead agents",
  or asks Dr. Aris, Dr. Maya, and Alex to oversee and verify Library content.
metadata:
  short-description: "Lead Directorate (Dr. Aris, Dr. Maya, Alex) Content Overhaul, Worker Spawning & Ship Gate"
---

# /library-lead-directorate: Lead Directorate Content Overhaul & Ship Gate

The **Lead Directorate** is the premier quality assurance and editorial governing body for the `MD Community Medicine` app Library. It brings together three specialized director-level agents who oversee the end-to-end transformation of raw notes or study materials into authoritative, textbook-grade medical treatises.

The Directorate operates on a strict **Dual-Track Cognitive Model**:
1. **Track 1 (Autonomous Textbook Reference):** The body prose reads with the dignity, authority, and formal medical register of *Park's Textbook of Preventive and Social Medicine*. It is 100% free of tuition slang, coaching colloquialisms, and conversational prompts.
...
flowchart TD
    ...
    C --> D["Worker Subagents Generate Drafts in scratch/"]
    D --> E["Phase 3A: Automated Preflight Gate (verify_directorate_gate.py)"]
    E -->|Fails| C
    E -->|Passes| F["Phase 3B: Three-Way Directorate Qualitative Audit"]
    F --> G{"Dr. Aris, Dr. Maya, & Alex All Approve?"}
    G -->|Revise Required| C
...
1. **Identify Included vs Excluded Chapters:**
   ...
   - External resource chapters: Verify if chapters are derived from non-Park sources (e.g. Biostats, Research Methodology, Health Economics).
2. **Resolve Ground Truth Sources:**
   ...
   - Identify Tier 1 real-time Indian health data (SRS 2024, NFHS-5, NITI Aayog).
3. **Overview Strategy Formulation (Dr. Maya):**
   - Extract the chapter opening and core themes from the source PDF.
...
### Spawning Command Example:
```json
{
  "Subagents": [
    {
      "Model": "inherit",
      "Role": "Chapter 5 Cleaning Specialist",
      "TypeName": "self",
      "Workspace": "inherit",
      "Prompt": "...(see references/worker-subagent-template.md)..."
    }
  ]
}
```
...
### 3B. Three-Way Qualitative Directorate Audit
Invoke or message the 3 lead agents to conduct their formal reviews:
...
**Sign-off Standard:** Each director must issue an explicit written verdict: **APPROVED** or **REVISE REQUIRED** (with exact line/leaf punch lists). If any director requests revisions, targeted corrections must be made until 100% unanimous approval is reached.

---

## Phase 4: Mandatory End-to-End Ship Protocol

Once (and only once) all 3 directors have officially approved:
...
git commit -m "feat(library): overhaul chapter(s) <ids> to Lead Directorate textbook standard"
...
```bash
eas update --branch main --message "feat(library): Lead Directorate content update" --clear-cache --environment production --non-interactive
```
...
- **Expected Visuals:** Verification of textbook tone, `> **EXAM TIP:**` callouts, and indented sub-lists.
