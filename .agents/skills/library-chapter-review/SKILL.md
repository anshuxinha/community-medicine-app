...
| Stop-slop skill | `.agents/skills/stop-slop/SKILL.md` (or `~/.gemini/config/skills/stop-slop/`) |
...
- Needs `pymupdf` (fitz) or `pdfplumber` (`py -3 -m pip install pymupdf`).
...
### Stop-slop filtering workflow (verbatim exception + clean prose)

Filter all proposed content additions and edits through the **stop-slop** skill rules (`.agents/skills/stop-slop/SKILL.md`):

1. **Tier 1 (Verbatim source content - preserved as-is):**
   - Standard definitions, statutory legal text, official WHO/MoHFW phrasing, diagnostic criteria lists, classification tables, and clinical cut-offs must remain **strictly verbatim from the source**.
   - Do NOT edit, soften, or alter verbatim source definitions with stop-slop.

2. **Tier 2 (Explanatory and framing prose - filtered through stop-slop):**
   - All following teaching paragraphs, clinical/epidemiological commentary, elaborations of each limb, and Exam Tip framing must pass through the stop-slop rules:
     - **Kill throat-clearing and filler openers:** Cut phrases like "It is worth noting that", "Crucially", "In essence", "At its core", "Importantly", "Furthermore, it should be noted that".
     - **Cut all adverbs:** Eliminate adverbs such as "significantly", "extremely", "notably", "essentially", "critically".
     - **Break formulaic structures:** Avoid binary contrasts ("not X, but Y"), negative listings, dramatic fragmentation, and false agency ("the system allows", "the strategy unfolds").
     - **Use direct active voice:** Name the clinician, epidemiologist, health worker, or patient doing the action. No passive voice.
     - **Be dense and specific:** Replace vague declaratives ("plays a vital role", "the implications are far-reaching") with concrete public health / clinical facts.
     - **Vary rhythm:** Mix sentence lengths; avoid metronomic 3-item lists and cliché one-liner paragraph endings.
...
- [ ] **Filtered through stop-slop:** active voice, zero AI fluff/filler, no unnecessary adverbs, specific facts; verbatim definitions/classifications preserved
...
5. **Pre-flight:** grep applied text for forbidden reader-offload phrases (`verify latest`, `exam year`, `confirm yourself`, `NEEDS_`, "commonly taught; confirm", etc.), **textbook name-dropping** (`Park`, `park-aligned`), **em-dashes** (U+2014, ` -- `), **`India hook`**, AI filler/throat-clearing phrases (`crucially`, `in essence`, `worth noting`, `pivotal role`, `moreover`), and live-text **`leaf`** as a section label. Remove all hits before ship. Confirm **acronyms expand on first use** in each changed subsection. Confirm definitions/categorisations are verbatim and that all non-verbatim explanatory prose conforms to the stop-slop skill rules.
...
- [ ] Pre-flight: no reader-offload phrases, no textbook name-dropping, no em-dashes, no stop-slop violations in non-verbatim prose, acronyms expanded on first use in shipped text
