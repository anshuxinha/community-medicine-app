...
9. **No meta language or PYQ Focus badges:** Never include author notes, prompt residue, or badges like `(PYQ FOCUS)`, `(PYQ Focus)`, or `(PYQ)` in headings, definition keys, or body text. Use dedicated `[SN]...[/SN]` and `[LAQ]...[/LAQ]` tag lines.
10. **Authoritative [REF] tags:** Every leaf ends with a clean `[REF]Reference: ...[/REF]` tag referencing the core standard textbooks (Park, Sathe, Sunder Lal) and official national guidelines.
...
### C5. Stop-slop prose filtering

Filter all proposed non-verbatim additions and edits through the **stop-slop** skill rules:
- **Verbatim parts:** Preserve exact source wording for definitions, statutory legal clauses, diagnostic criteria lists, classification tables, and clinical cut-offs.
- **Non-verbatim parts:** Scrub all following teaching explanations, clinical commentary, section transitions, and Exam Tip framing of AI writing patterns:
  - Eliminate throat-clearing openers ("It is worth noting that", "Crucially", "In essence", "At its core", "Importantly").
  - Cut all adverbs ("significantly", "extremely", "notably", "essentially", "critically").
  - Break formulaic structures ("not X, but Y", negative listings, dramatic fragmentation, false agency).
  - Use direct active voice (name the clinician, epidemiologist, health worker, or patient).
  - Be specific and dense; eliminate vague declaratives ("plays a crucial role", "has profound implications").
  - Vary sentence rhythm; no em-dashes.
...
| Stop-slop compliance | All added/modified non-verbatim prose is filtered through stop-slop rules (active voice, zero AI filler/adverbs, concrete facts; verbatim definitions/classifications preserved). |
...
| Mnemonic tone | Only next to real lists; clear letter expansions; accurate over clever. Prefer **4–10 items**. No em-dashes |
...
- Violate stop-slop rules in non-verbatim prose (no AI throat-clearing, no filler adverbs, no passive voice, no vague declaratives).
