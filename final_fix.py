...
def reformat_content(text, topic_title):
    ...
    while i < len(lines):
        ...
        if (stripped == stripped.upper() and 
            len(stripped) > 3 and
            any(c.isalpha() for c in stripped)):  # Must have at least one letter
        ...
        if is_heading:
            # Add blank line before (if not start of content)
            if result and result[-1] != '':
                result.append('')
...
print("Reformating content fields (FINAL FIX)...")
