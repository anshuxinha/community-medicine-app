...
const REFERENCE_VALUES = {
  man_sedentary: {
    label: "Reference Man (Sedentary)",
    kcal: 2110,
    protein: 54,
    fat: 70,
  },
  woman_sedentary: {
    label: "Reference Woman (Sedentary)",
    kcal: 1660,
    protein: 46,
    fat: 55,
  },
  man_moderate: {
    label: "Reference Man (Moderate)",
    kcal: 2710,
    protein: 54,
    fat: 90,
  },
  woman_moderate: {
    label: "Reference Woman (Moderate)",
    kcal: 2130,
    protein: 46,
    fat: 71,
  },
  man_heavy: {
    label: "Reference Man (Heavy)",
    kcal: 3470,
    protein: 54,
    fat: 116,
  },
  woman_heavy: {
    label: "Reference Woman (Heavy)",
    kcal: 2720,
    protein: 46,
    fat: 91,
  },
...
const DietarySurveyScreen = () => {
  ...
  const addRow = () =>
    setRows([...rows, { foodId: foodData[0].id, grams: "" }]);
  ...
  const calculate = () => {
    ...
    setResult({
      ...
      proteinDiff: (((totalProtein - ref.protein) / ref.protein) * 100).toFixed(
        1,
      ),
              ...
              {[
                {
                  label: "Calories",
                  unit: "kcal",
                  got: result.kcal.toFixed(0),
                  ref: result.ref.kcal,
                  diff: result.kcalDiff,
                },
                {
                  label: "Protein",
                  unit: "g",
                  got: result.protein.toFixed(1),
                  ref: result.ref.protein,
                  diff: result.proteinDiff,
                },
                {
                  label: "Fat",
                  unit: "g",
                  got: result.fat.toFixed(1),
                  ref: result.ref.fat,
                  diff: result.fatDiff,
                },
              ].map((row) => (
                <View key={row.label} style={styles.resultRow}>
                  ...
                  <Text
                    style={{ color: diffColor(row.diff), fontWeight: "bold" }}
                  >
                    {diffLabel(row.diff)}
                  </Text>
                </View>
