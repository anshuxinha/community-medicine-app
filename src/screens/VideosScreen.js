...
<html>
  <head>
    ...
    <style>
      ...
      /* Gesture overlay covers top 85% of the video player, leaving bottom 15% for controls */
      .gesture-overlay {
        position: absolute;
        top: 0;
        left: 0;
      ...
      @keyframes ripple-left-anim {
        0% {
          transform: translate(-80%, -50%) scale(0.2);
          opacity: 0.8;
      ...
      @keyframes ripple-right-anim {
        0% {
          transform: translate(-20%, -50%) scale(0.2);
          opacity: 0.8;
      ...
      .ripple-left {
        animation: ripple-left-anim 0.6s cubic-bezier(0.1, 0.8, 0.3, 1) forwards;
      }

      .ripple-right {
        animation: ripple-right-anim 0.6s cubic-bezier(0.1, 0.8, 0.3, 1) forwards;
      }
    </style>
    <!-- Include Player.js to communicate with Bunny.net stream player -->
  ...
  <body>
    <div class="player-container">
      ...
      <div class="gesture-overlay">
        <div class="gesture-zone zone-left" onclick="onZoneClick(event, 'left')">
          <div id="left-feedback" class="feedback-indicator">
        ...
        <div class="gesture-zone zone-center" onclick="onZoneClick(event, 'center')">
          <div id="center-feedback" class="feedback-indicator">
            <div class="center-indicator" id="center-icon-container">
              <!-- SVG will be injected here (play or pause) -->
            </div>
          </div>
        </div>
        
        <div class="gesture-zone zone-right" onclick="onZoneClick(event, 'right')">
          <div id="right-feedback" class="feedback-indicator">
            <div class="side-indicator">
              <svg viewBox="0 0 24 24">
                <!-- Double right arrow -->
    ...
    <script>
      ...
      function onZoneClick(event, zone) {
        tapCount++;
        
        if (tapCount === 1) {
          tapTimeout = setTimeout(() => {
            tapCount = 0;
            handleSingleTap(event, zone);
          }, 250);
        } else if (tapCount === 2) {
          clearTimeout(tapTimeout);
          tapCount = 0;
          handleDoubleTap(event, zone);
        }
      }

      function handleSingleTap(event, zone) {
        if (!isReady) return;
        
        // Single tap in the middle toggles play/pause
        if (zone === 'center') {
      ...
      function handleDoubleTap(event, zone) {
        if (!isReady) return;
        ...
        if (zone === 'left') {
          ...
          player.getCurrentTime((time) => {
            ...
            showSideFeedback('left', event);
          });
        } else if (zone === 'right') {
          // Seek forward 10s
          player.getCurrentTime((time) => {
            player.getDuration((duration) => {
              player.setCurrentTime(Math.min(duration || 99999, time + 10));
              showSideFeedback('right', event);
            });
          });
        }
      }
      ...
      let centerTimeout;
      function showCenterFeedback(action) {
        const feedback = document.getElementById('center-feedback');
        const container = document.getElementById('center-icon-container');
        
        container.innerHTML = action === 'play' ? playIcon : pauseIcon;
      ...
      let sideTimeouts = { left: null, right: null };
      function showSideFeedback(side, event) {
        const feedback = document.getElementById(side + '-feedback');
        
        // Trigger ripple
        createRipple(side, event);
        
        feedback.classList.add('active');
        clearTimeout(sideTimeouts[side]);
        
        sideTimeouts[side] = setTimeout(() => {
          feedback.classList.remove('active');
        }, 650);
      }

      function createRipple(side, event) {
        const zone = document.querySelector('.zone-' + side);
        const ripple = document.createElement('div');
        ripple.classList.add('ripple-effect');
        ripple.classList.add(side === 'left' ? 'ripple-left' : 'ripple-right');
        
        // Position relative to the zone
